                                ...
                                <div class="col-lg-8 offset-lg-2" style="margin-top: 2%">
                                    <div class="card shadow mb-4">
                                        <div class="card-header py-3">
                                            <h6 class="m-0 font-weight-bold text-primary">Editar de Publicacion</h6>
                                        </div>
                                        <div class="card-body">
                                            <form action="Publicaciones" method="post" class="user needs-validation" novalidate>
                                                <input type="hidden" name="idPublicacion" 
                                                       value="<jsp:getProperty name="modeloPublicacion" property="id"></jsp:getProperty>" />
                                                       <div class="form-group">
                                                           <input name="txtTitulo" type="text" class="form-control form-control-user" placeholder="Titulo" required maxlength="50" 
                                                           value="<jsp:getProperty name="modeloPublicacion" property="titulo"></jsp:getProperty>" />
                                                           <div class="invalid-feedback">Debe ingresar un titulo</div>
                                                       </div>
                                                       <div class="form-group">
                                                           <textarea name="txtContenido" class="form-control" placeholder="Contenido" rows="3"
                                                                     ><jsp:getProperty name="modeloPublicacion" property="contenido"></jsp:getProperty></textarea
                                                       </div>
                                                       <div class="form-group">
                                                           ...
                                                           <c:forEach items="${ listaActividades }" var="item">
                                                               <option value="${ item.id }" <c:if test="${ item.id == modeloPublicacion.getActividad().getId()}">selected</c:if>>${ item.nombre }</option>
                                                           </c:forEach>
                                                       </select>
                                                ...
                                                <div class="form-group">
                                                    <input name="txtUrlImagen" class="form-control form-control-user" placeholder="Url de imagen"
                                                           value="<jsp:getProperty name="modeloPublicacion" property="urlImagen"></jsp:getProperty>" />
                                                    </div>
                                                    <div class="form-group">
                                                        <input name="txtFecha" type="date" class="form-control form-control-user" placeholder="DD/MM/AAAA" required
                                                               value="<jsp:getProperty name="modeloPublicacion" property="fechaString"></jsp:getProperty>" />
                                                        <div class="invalid-feedback">Debe seleccionar una fecha</div>
                                                    </div>
                                                    <div class="form-group">

                                                        <div class="form-check">
                                                            <input name="chkActiva" class="form-check-input" type="checkbox" id="chkActiva" value="true" 
                                                                <c:if test="${ modeloPublicacion.activa == true}">checked</c:if> />
